...
RSpec.describe "Users::Registrations", type: :request do
  ...
  describe "#edit" do
    let(:url) { get edit_user_registration_path }
  ...
  describe "#update" do
    let(:url) do
      patch update_user_registration_path(user), params: { user: { username: "アムロ" } }
    end
    ...
    context "ログインユーザー" do
      ...
      it { is_expected.to redirect_to mypage_path(user) }
      it "正常に更新されること" do
        expect(user.reload).to have_attributes(username: "アムロ")
      end
    end
    ...
    context "未ログインユーザー" do
      ...
      it "更新されないこと" do
        expect(user.reload).not_to have_attributes(username: "アムロ")
      end
    end
    ...
    context "他ユーザーを指定" do
      ...
      it "更新されないこと" do
        expect(user.reload).not_to have_attributes(username: "アムロ")
        expect(other_user.reload).to have_attributes(username: "アムロ")
      end
    end
  ...
  describe "#new_confirm" do
    ...
    context "アカウント未有効化ユーザー" do
      let(:url) do
        post signup_confirm_path, params: {
          user: {
            username: unconfirm.username,
            email: unconfirm.email,
            password: unconfirm.password,
            password_confirmation: unconfirm.password,
          },
        }
      end

      let(:unconfirm) { create(:user, :unconfirmation) }
      let(:login) { nil }
  ...
  describe "#edit_email" do
    let(:url) { get edit_email_user_registration_path }
  ...
  describe "#update_email" do
    let(:url) do
      patch update_email_user_registation_path, params: {
        user: {
          email: "new@example.com",
          email_confirmation: "new@example.com",
        },
  ...
  describe "#update_password" do
    let(:url) do
      patch update_password_user_registration_path, params: {
        user: {
          password: "new-password",
          password_confirmation: "new-password",
